...
SyMethod::post_execute (SymbolTable *table)
{
  ...
  if (t && t->m_type != s_return)
    table->add_error (head->lineno, "'" + s_id + "' has wrong return type ('" + t->m_type
                                        + "' and '" + s_return + "')");
}
...
SyAssign::post_execute (SymbolTable *table)
{
  ...
  if (l->m_type != s_type)
    table->add_error (lineno, "'" + s_id + "'(" + l->m_type + ") and '" + s_type
...
SyAssignArr::post_execute (SymbolTable *table)
{
  ...
  s_type = (*this)[1]->get_type (table);
  if (s_type != "int")
    table->add_error ((*this)[0]->lineno,
                      "wrong type in int array operation [], expected 'int' got '" + s_type + "'");

  s_type = (*this)[2]->get_type (table);
  if (s_type != "int")
    table->add_error ((*this)[2]->lineno,
                      "can not assign value of type '" + s_type + "' to an integer array");
...
SyAttribute::post_execute (SymbolTable *table)
{
  ...
  if (!m_param.empty () && this->value == "attr")
    table->add_error (this->lineno, "'" + s_attribute + "' requiers " + to_string (m_param.size ())
                                        + " arguments but non where given");
  else if (this->value == "attr_exp")
    {
      ...
      if (args.size () > m_param.size ())
        table->add_error ((this->lineno), "to many arguments were given, expected "
                                              + to_string (m_param.size ()) + " argument(s) for '"
                                              + s_attribute + "', but " + to_string (args.size ())
                                              + " were given.");
      ...
      for (int i = 0; i < m_param.size (); i++)
        {
          if (i >= args.size ())
            {
              table->add_error ((this->lineno), "missing " + to_string (m_param.size () - i)
                                                    + " argument(s) for '" + s_attribute + "'.");
              break;
            }

          if (args[i] != m_param[i]->m_type)
            table->add_error ((this->lineno), "invalid argument for '" + s_attribute
                                                  + "' at position: " + to_string (i)
                                                  + " expected '" + m_param[i]->m_type
                                                  + "' but got '" + args[i] + "'");
        }
...
SyArrayPull::post_execute (SymbolTable *table)
{
  ...
  if (s_type != "int")
    table->add_error ((*this)[1]->lineno,
                      "wrong type in int array expression [], expected 'int' got '" + s_type + "'");
...
SyNot::post_execute (SymbolTable *table)
{
  ...
  if (s_type != "boolean")
    table->add_error ((*this)[0]->lineno,
                      "Expression inside 'not' operation is not of type 'boolean'");
