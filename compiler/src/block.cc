...
void
Block::generate_code (ofstream *out, SymbolTable *table)
{
  for (TAC *t : m_tacs)
    t->generate_code (out);
  ...
  if (m_false_exit)
    m_false_exit->generate_code (out, table);
}
...
void
IfBlock::generate_code (std::ofstream *out, SymbolTable *table)
{
  for (TAC *t : m_tacs)
    t->generate_code (out);
...
void
IfElseBlock::generate_code (std::ofstream *out, SymbolTable *table)
{
  for (TAC *t : m_tacs)
    t->generate_code (out);
...
void
WhileBlock::generate_code (std::ofstream *out, SymbolTable *table)
{
...
void
MethodBlock::generate_code (std::ofstream *out, SymbolTable *table)
{
  Scope *scope;
...
void
JoinBlock::generate_code (std::ofstream *out, SymbolTable *table)
{
  m_counter++;
